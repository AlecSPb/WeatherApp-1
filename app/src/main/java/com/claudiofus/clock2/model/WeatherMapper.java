...
public class WeatherMapper {
    ...
    public String getConditionFromCode(int code){
        if (Constants.getWeatherCondition().size() > 0) {
            if (sunnyList.contains(code)) {
                return Constants.getWeatherCondition().get(0);
            } else if (rainyList.contains(code)) {
                return Constants.getWeatherCondition().get(1);
            } else if (cloudyList.contains(code)) {
                return Constants.getWeatherCondition().get(2);
            } else if (snowyList.contains(code)) {
                return Constants.getWeatherCondition().get(3);
            } else if (foggyList.contains(code)) {
                return Constants.getWeatherCondition().get(4);
            } else if (windyList.contains(code)) {
                return Constants.getWeatherCondition().get(5);
            }
        }
        return "";
    }
